...
: Node("simple_pure_pursuit"),
  ...
  cuvature_thresh(declare_parameter<float>("cuvature_thresh", 0.1)),
  velocity_offset(declare_parameter<float>("velocity_offset", 0.0)),
  acc_offset(declare_parameter<float>("acc_offset", 0.0))
{
...
void SimplePurePursuit::onTimer()
{
  ...
  if (
    ...
    double curvature = std::abs(2.0 * std::sin(alpha) / lookahead_distance);
    if(curvature > cuvature_thresh && current_longitudinal_vel >= 3.5){
      cmd.longitudinal.speed =  odometry_->twist.twist.linear.x + velocity_offset;
      // 減速具合の調整．上の速度は進行方向のみ　
      cmd.longitudinal.acceleration = 0 + acc_offset;
    }
  }
